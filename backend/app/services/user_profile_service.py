...
class UserProfileService:
    ...
    def create_profile(self, profile_in: UserProfileCreate) -> UserProfile:
        ...
        if existing:
            raise ValueError(f"Profile already exists for user")
    ...
    def get_profiles(self, skip: int = 0, limit: int = 100) -> tuple[list[UserProfile], int]:
        """Get all profiles with count"""
        profiles = self.repository.get_all(skip=skip, limit=limit)
        count = self.repository.count()
        return profiles, count

    def update_profile(self, db_profile: UserProfile, profile_in: UserProfileUpdate) -> UserProfile:
    ...
    def assign_site_to_profile(
        ...
        return self.profile_site_repository.assign_site(profile_id, site_id, role_in_site)

    def remove_site_from_profile(self, profile_id: uuid.UUID, site_id: uuid.UUID) -> bool:
